...
# Este script é o mesmo da instalação original, porem com uma pequena modificação na rotina
# setDBAPassword() onde é setado a senha do SYSDBA, como estamos rodando no modo silencioso
# o que é obrigatório na criação da imagem, por default na instalação original é gerado uma senha
# randomica qualquer, neste caso não queremos isto, então foi modificado para respeitar o parâmetro
# ISC_PASSWORD setado no arquivo de constantes
# Se você trocar a versão do Firebird utilizada, recomendo que troque este arqivo pelo equivalente da mesma versão,
# porem com a modificação que aqui foi realizada.
